...
def load_models():
    ...
    for call_name, real_name in model_name_dict.items():
        ...
        model = GPTJForCausalLM.from_pretrained(real_name, revision="float16", torch_dtype=torch.float16, low_cpu_mem_usage=True)
...
def GenerateText(text,tokens):
    ...
    output = generator(text, do_sample=True, temperature=0.9, max_length=100, max_new_tokens=tokens)
