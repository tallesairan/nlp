...
def load_models():
    ...
    for call_name, real_name in model_name_dict.items():
        ...
        model = GPTJForCausalLM.from_pretrained(real_name, revision="float16", torch_dtype=torch.float16, low_cpu_mem_usage=True).to("cuda",  torch_dtype=torch.float16)
        # model = AutoModelForCausalLM.from_pretrained(real_name,  torch_dtype=torch.float16, low_cpu_mem_usage=True)
        tokenizer = AutoTokenizer.from_pretrained(real_name).to("cuda", torch_dtype=torch.float16)
